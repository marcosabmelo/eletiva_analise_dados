## REGRAS DE ASSOCIAÇÃO COM R ##
# Marcos A. B. de Melo
# 27/09/2023

# Carregando pacotes
#---------------------------------------------------
pacman::p_load(data.table, dplyr, caret, arules, arulesCBA, arulesViz, reactablefmtr)

# Lendo base do TSE
#---------------------------------------------------
candidatos_pe_2022 <- fread('consulta_cand_2022_PE.csv', encoding = 'Latin-1', stringsAsFactors = T)

# filtrando apenas deputados federais e variáveis de perfil
#---------------------------------------------------
federais_pe_2022 <- candidatos_pe_2022 %>%
...
# observando se os dados estão com as classes certas
#---------------------------------------------------
glimpse(federais_pe_2022)

# modificando nomes para facilitar análise e plot
#---------------------------------------------------
names(federais_pe_2022) <- c('agremiacao', 'municipio', 'idade', 'genero',
                              'instrucao', 'estado_civil', 'raca', 'ocupacao')

# discretizando variável numérica
#---------------------------------------------------
federais_pe_2022[ , "idade"] <- discretizeDF(federais_pe_2022[ , "idade"]) # transforma variáveis numéricas em fatores
